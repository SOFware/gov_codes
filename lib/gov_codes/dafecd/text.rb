...
module GovCodes
  module Dafecd
    ...
    module Text
      ...
      def split_glued_afsc(text)
        text.gsub(/(?<=[A-Za-z])(?=AFSC \d[A-Z]\d\d\d)/, "\n")
      end

      # Normalize a DAFECD date such as "31 Oct 25" or "31 Oct 2024" to ISO 8601
      # ("2025-10-31" / "2024-10-31"). Returns nil when unparseable.
      def parse_date(raw)
